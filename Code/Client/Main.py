...
class MyWindow(QMainWindow, Ui_client):
    def __init__(self):
        ...
        self.Button_Dead.pressed.connect(self.dead)
    ...
    def dead(self):
        ...
        try:
            print(self.Button_Dead.text())
            print(cmd.CMD_DEAD)
            self.client.send_data(cmd.CMD_DEAD+'#1'+'\n')

            if self.Button_Dead.text() == 'Dead':
                self.Button_Dead.setText('Died')
                command = cmd.CMD_DEAD + '#1' + '\n'
                self.client.send_data(command)
            else:
                self.Button_Dead.setText('Dead')
                command = cmd.CMD_DEAD + '#0' + '\n'
            print(command)
            self.client.send_data(command)
        except Exception as e:
            print(e)
    ...
    def attack(self, servo, action):
        try:
            if type(servo) == int:
                command = cmd.CMD_ATTACK + '#' + str(servo) + '#' + str(action) + '\n'
            else:
                'Non-existent servo. Add cheese'
...
class calibrationWindow(QMainWindow, Ui_calibration):
    ...
    def X1(self):
        self.get_point()
        self.x += 1
        command = cmd.CMD_CALIBRATION + '#' + self.leg + '#' + str(self.x) + '#' + str(self.y) + '#' + str(
            self.z) + '\n'
        self.client.send_data(command)
        self.set_point()
