...
class Control:
    ...
    def calibration(self):
        ...
        for i in range(6):
            self.calibration_angle[i][0], self.calibration_angle[i][1], self.calibration_angle[i][
                2] = self.coordinateToAngle(-self.calibration_leg_point[i][2],
    ...
    def setLegAngle(self):
        if self.checkPoint():
            ...
            for i in range(3):
                ...
                self.angle[i + 3][1] = self.restriction(90 + self.angle[i + 3][1] + self.calibration_angle[i + 3][1], 0,
                                                        180)
                self.angle[i + 3][2] = self.restriction(180 - (self.angle[i + 3][2] + self.calibration_angle[i + 3][2]),
                                                        0, 180)
    ...
    def condition(self):
        while True:
            ...
            elif cmd.CMD_DEAD in self.order:
                if self.order[1] == "1":
                    print("Dying!")

                    # leg1
                    self.servo.setServoAngle(15, 90)
                    self.servo.setServoAngle(14, 172)
                    self.servo.setServoAngle(13, 1)

                    # leg2
                    self.servo.setServoAngle(12, 100)
                    self.servo.setServoAngle(11, 167)
                    self.servo.setServoAngle(10, 1)

                    # leg3
                    self.servo.setServoAngle(9, 75)
                    self.servo.setServoAngle(8, 157)
                    self.servo.setServoAngle(31, 1)

                    # leg4
                    self.servo.setServoAngle(22, 85)
                    self.servo.setServoAngle(23, 13)
                    self.servo.setServoAngle(27, 174)

                    # leg5
                    self.servo.setServoAngle(19, 95)
                    self.servo.setServoAngle(20, 13)
                    self.servo.setServoAngle(21, 179)

                    # leg6
                    self.servo.setServoAngle(16, 95)
                    self.servo.setServoAngle(17, 13)
                    self.servo.setServoAngle(18, 184)
                else:
                    self.setLegAngle()
    ...
    def coordinateTransformation(self, point):
        ...
        # leg3
        self.leg_point[2][0] = point[2][0] * math.cos(math.radians(-54)) + point[2][1] * math.sin(
            math.radians(-54)) - 94
        self.leg_point[2][1] = -point[2][0] * math.sin(math.radians(-54)) + point[2][1] * math.cos(math.radians(-54))
        self.leg_point[2][2] = point[2][2] - 14
        # leg4
        self.leg_point[3][0] = point[3][0] * math.cos(math.radians(-126)) + point[3][1] * math.sin(
            math.radians(-126)) - 94
        ...
        # leg5
        self.leg_point[4][0] = point[4][0] * math.cos(math.radians(180)) + point[4][1] * math.sin(
            math.radians(180)) - 85
        ...
        # leg6
        self.leg_point[5][0] = point[5][0] * math.cos(math.radians(126)) + point[5][1] * math.sin(
            math.radians(126)) - 94
    ...
    def run(self, data, Z=40, F=64):  # example : data=['CMD_MOVE', '1', '0', '25', '10', '0']
        ...
        for i in range(6):
            xy[i][0] = ((point[i][0] * math.cos(math.radians(angle)) + point[i][1] * math.sin(math.radians(angle)) -
                         point[i][0]) + x) / F
            xy[i][1] = ((-point[i][0] * math.sin(math.radians(angle)) + point[i][1] * math.cos(math.radians(angle)) -
                         point[i][1]) + y) / F
